...
guards' military intelligences are trained to understand." CR>
		...
		<RTRUE>)
	       (<AND <VERB? EXAMINE>
		     <IN? ,RIFLES ,GUARDS>>
		<TELL "They're holding their photon rifles." CR>)
	       (<AND <EQUAL? ,HERE ,DAIS>		     
		     <VERB? SHOOT>>
		<COND (<BLASTER-HOLD>
		       <RTRUE>)>
...
<OBJECT FRUSTATION
	(LOC GLOBAL-OBJECTS)
	(DESC "problem")
	(SYNONYM FRUSTRATI PROBLEM PUZZLE)
...
<ROOM DARK
      (LOC ROOMS)
      (SYNONYM SUNGLASSE GLASSES JANTA)
...
<OBJECT DARK-OBJECT
	(LOC GLOBAL-OBJECTS) ;"gets moved to Dark when you turn on the light"
	(DESC "darkness")
...
<OBJECT LIQUID
	...
	(DESC "liquid")
	(SYNONYM LIQUID)
	(ADJECTIVE HOT COLD WARM WET SQUISHY)
...
<ROUTINE DARK-FUNCTION ()
	 <COND (<VERB? QUIT RESTART RESTORE SCORE VERSION SAVE ;AGAIN HINTS
...
<OBJECT TOOTHBRUSH
	(LOC BEDROOM)
	(DESC "toothbrush")
	(SYNONYM TOOTHBRUS BRUSH TOOL TOOLS)
...
<OBJECT WRENCH
	...
	(ADJECTIVE LASER ASSISTED LASER-ASS MONKEY PROPER)
...
<OBJECT CHISEL
	...
	(ADJECTIVE THERMO FUSION THERMO-FU PROPER)
	(FLAGS TAKEBIT TOOLBIT)
	(SIZE 3)
	(GENERIC TWEEZERS)>
...
<OBJECT AWL
	...
	(ADJECTIVE ULTRA PLASMIC ULTRA-PLA VACUUM PROPER)
...
<OBJECT TWEEZERS
	...
	(SYNONYM TWEEZERS TWEEZER PAIR TOOL TOOLS)
	(ADJECTIVE PROPER)
	(FLAGS TAKEBIT TOOLBIT)
	(SIZE 3)
	(GENERIC TWEEZERS)>
...
<OBJECT PINCER
	...
	(ADJECTIVE MOLECULAR HYPERWAVE PROPER)
	(FLAGS TAKEBIT TOOLBIT)
	(SIZE 3)
	(GENERIC TWEEZERS)>

<OBJECT RASP
	(LOC LOCAL-GLOBALS) ;"for the sake of MOBY-FIND"
	(DESC "ionic diffusion rasp")
	(SYNONYM RASP TOOL TOOLS)
	(ADJECTIVE IONIC DIFFUSION PROPER)
	(FLAGS TAKEBIT VOWELBIT TOOLBIT)
	(SIZE 3)
	(GENERIC TWEEZERS)> 
...
<OBJECT CHIPPER
	...
	(SYNONYM CHIPPER TOOL TOOLS)
	(ADJECTIVE NUMBER TWELVE ASTEROID PAINT PROPER)
	(FLAGS TAKEBIT TOOLBIT)
	(SIZE 3)
	(GENERIC TWEEZERS)>
...
<OBJECT THUMB
	...
	(ADJECTIVE ELECTRONI SUB-ETHA SIGNALING SMALL BLACK LITTLE BLINKING)
	(SIZE 10)
...
<OBJECT GREEN-BUTTON
	...
	(DESC "green button")
	(SYNONYM BUTTON)
	(ADJECTIVE GREEN SMALL HITCHHIKE)
	(FLAGS NDESCBIT INTEGRALBIT)
	(SIZE 1)
...
<OBJECT GUARANTEE
	(LOC THUMB)
	(DESC "guarantee")
	(SYNONYM GUARANTEE WARRANTY PLAQUE)
	(ADJECTIVE LIFETIME)
	(FLAGS NDESCBIT INTEGRALBIT)
	(SIZE 2)
	(ACTION GUARANTEE-F)>
...
<CONSTANT ASK-ABOUT-OBJECT "\"Oh...you're trying to figure that out also? The
manual's not much help, is it? By the way, do you know your score? I don't.
My computer doesn't have a status line.\"">

<CONSTANT DRIVEN-BACK "You struggle to reach the Thumb, but the wind is too
fierce and you are driven back.">

<CONSTANT GUARDS-REALIZE "A wisp of an inkling of a thought penetrates the
...
<CONSTANT DIALING-TONE " A moment later, the dialing tone is suddenly cut off.
...
<CONSTANT THUMB-FUMBLE "You fumble with the Thumb as you hold onto the tree
