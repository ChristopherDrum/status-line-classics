<DEFAULTS-DEFINED DEBUGGING-CODE COMMAND-CODE>

; "Delete the REPLACE-DEFINITION in MISC.ZIL to cause this to be included"
<DEFAULT-DEFINITION DEBUGGING-CODE

<CONSTANT GL-SPACES-40 "                                        ">

<CONSTANT GL-IT-DIRS:TABLE
	<PTABLE
		P?NORTH
		P?NE
		P?EAST
		P?SE
		P?SOUTH
		P?SW
		P?WEST
		P?NW
		P?UP
		P?DOWN
		P?IN
		P?OUT>>

<ROUTINE RT-CONTEXT-STR (CONTEXT "AUX" STR)
	<COND
		(<EQUAL? .CONTEXT 0>
			<SET STR "None">
		)
		(<EQUAL? .CONTEXT 1>
			<SET STR "Begin">
		)
		(<EQUAL? .CONTEXT 2>
			<SET STR "Entering">
		)
		(<EQUAL? .CONTEXT 3>
			<SET STR "Entered">
		)
		(<EQUAL? .CONTEXT 4>
			<SET STR "Exiting">
		)
		(<EQUAL? .CONTEXT 5>
			<SET STR "End">
		)
		(<EQUAL? .CONTEXT 6>
			<SET STR "FLook">
		)
		(<EQUAL? .CONTEXT 7>
			<SET STR "NLook">
		)
		(<EQUAL? .CONTEXT 8>
			<SET STR "Winner">
		)
		(<EQUAL? .CONTEXT 9>
			<SET STR "CONTAINER">
		)
		(<EQUAL? .CONTEXT 10>
			<SET STR "Object Desc">
		)
		(<EQUAL? .CONTEXT 99>
			<SET STR "Fatal">
		)
		(T
			<SET STR "???">
		)
	>
	<RETURN .STR>>

<ROUTINE RT-DIRECTION-STR (DIRECTION "AUX" STR)
	<COND
		(<EQUAL? .DIRECTION 1>
			<SET STR "N">
		)
		(<EQUAL? .DIRECTION 2>
			<SET STR "NE">
		)
		(<EQUAL? .DIRECTION 3>
			<SET STR "E">
		)
		(<EQUAL? .DIRECTION 4>
			<SET STR "SE">
		)
		(<EQUAL? .DIRECTION 5>
			<SET STR "S">
		)
		(<EQUAL? .DIRECTION 6>
			<SET STR "SW">
		)
		(<EQUAL? .DIRECTION 7>
			<SET STR "W">
		)
		(<EQUAL? .DIRECTION 8>
			<SET STR "NW">
		)
		(<EQUAL? .DIRECTION 9>
			<SET STR "UP">
		)
		(<EQUAL? .DIRECTION 10>
			<SET STR "DOWN">
		)
		(<EQUAL? .DIRECTION 11>
			<SET STR "IN">
		)
		(<EQUAL? .DIRECTION 12>
			<SET STR "OUT">
		)
		(T
			<SET STR "???">
		)
	>
	<RETURN .STR>>

<ROUTINE RT-SAY-WORD (WORD)
	<COND
		(<MC-F? .WORD>
			<TELL "...">
		)
		(T
			<PRINTB .WORD>
		)
	>>

<ROUTINE RT-SAY-IT (OBJ "AUX" (CNT 0))
	<REPEAT ()
		<COND
			(<EQUAL? .CNT 12>
				<RETURN>
			)
			(<EQUAL? .OBJ <GET ,GL-IT-DIRS .CNT>>
				<TELL <RT-DIRECTION-STR .OBJ>>
				<RTRUE>
			)
		>
		<INC CNT>
	>
	<COND
		(<EQUAL? .OBJ ,ROOMS>
			<TELL "Rooms">
		)
		(<MC-T? .OBJ>
			<TELL D .OBJ>
		)
		(T
			<TELL "...">
		)
	>>

<ROUTINE RT-DEBUG-PARSER (VERBSTR)
	<COND (<MC-T? ,GL-DEBUG-FLAG>
	       ;<BUFOUT <>>
	       <SCREEN ,K-S-WIN>
	       <CURSET 1 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 2 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 3 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 4 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 5 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 6 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 7 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 8 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 9 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 10 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 11 1>
	       <TELL ,GL-SPACES-40>
	       <CURSET 1 1>
	       <TELL "VERB  :">
	       <TELL .VERBSTR>
	       <CURSET 2 1>
	       <TELL "MULT? :">
	       <COND (<MC-F? ,GL-P-MULT?>
		      <TELL "F">)
		     (T 
		      <TELL "T">)>
	       <CURSET 3 1>
	       <TELL "WINNER:">
	       <RT-SAY-IT ,GL-WINNER>
	       <CURSET 4 1>
	       <TELL "DIR   :">
	       <RT-SAY-WORD ,GL-P-DIR-WORD>
	       <CURSET 5 1>
	       <TELL "PRSO  :">
	       <RT-SAY-IT ,GL-PRSO>
	       <CURSET 6 1>
	       <TELL "PRSI  :">
	       <RT-SAY-IT ,GL-PRSI>
	       <CURSET 7 1>
	       <TELL "A-WORD:">
	       <RT-SAY-WORD ,GL-P-PRSA-WORD>
	       <CURSET 8 1>
	       <TELL "O-ADJ :">
	       <RT-SAY-WORD <GET ,GL-P-ADJW 0>>
	       <CURSET 9 1>
	       <TELL "O-NOUN:">
	       <RT-SAY-WORD <GET ,GL-P-NAMW 0>>
	       <CURSET 10 1>
	       <TELL "I-ADJ :">
	       <RT-SAY-WORD <GET ,GL-P-ADJW 1>>
	       <CURSET 11 1>
	       <TELL "I-NOUN:">
	       <RT-SAY-WORD <GET ,GL-P-NAMW 1>>
	       <SCREEN ,K-S-NOR>
	       ;<BUFOUT T>)>
	<RFALSE>>

<ROUTINE RT-DEBUG-XX-XX (STR1 STR2 CONTEXT "AUX" STR3)
	<COND
		(<MC-T? ,GL-DEBUG-FLAG>
			;<BUFOUT <>>
			<SCREEN ,K-S-WIN>

			<SET STR3 <RT-CONTEXT-STR .CONTEXT>>

			<CURSET 1 41>
			<TELL ,GL-SPACES-40>
			<CURSET 2 41>
			<TELL ,GL-SPACES-40>

			<CURSET 1 41>
			<TELL .STR1  ":" .STR2>
			<CURSET 2 41>
			<TELL "Cntx" ":" .STR3>

			<SCREEN ,K-S-NOR>
			;<BUFOUT T>
		)
	>
	<RFALSE>>

<ROUTINE RT-DEBUG-PRE-VERB (VERBSTR)
	<RT-DEBUG-PARSER .VERBSTR>>

<ROUTINE RT-DEBUG-VERB (VERBSTR)
	<RT-DEBUG-PARSER .VERBSTR>>

<ROUTINE RT-DEBUG-OB-AC (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "OBAC" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-OB-DF (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "OBDF" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-RM-AC (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "RMAC" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-RM-DF (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "RMDF" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-LG-AC (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "LGAC" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-LG-DF (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "LGDF" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-TH-AC (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "THAC" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-TH-DF (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "THDF" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-CH-AC (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "CHAC" .STR1 .CONTEXT>>

<ROUTINE RT-DEBUG-CH-DF (STR1 "OPTIONAL" (CONTEXT 0))
	<RT-DEBUG-XX-XX "CHDF" .STR1 .CONTEXT>>

<SYNTAX XEDEBUG = V-ENB-DEBUG>
<SYNTAX $EDEBUG = V-ENB-DEBUG>
<SYNTAX XDDEBUG = V-REFRESH>
<SYNTAX $DDEBUG = V-REFRESH>
<SYNTAX XGETLIT = V-GETLIT>
<SYNTAX $GETLIT = V-GETLIT>
<SYNTAX XGOSIGNA = V-GOSIGNA>
<SYNTAX $GOSIGNA = V-GOSIGNA>

<SYNTAX XGOSIGNB = V-GOSIGNB>
<SYNTAX $GOSIGNB = V-GOSIGNB>

<SYNTAX XGOSIGNC = V-GOSIGNC>
<SYNTAX $GOSIGNC = V-GOSIGNC>

<SYNTAX XGOSIGND = V-GOSIGND>
<SYNTAX $GOSIGND = V-GOSIGND>

<SYNTAX XGOSIGNE = V-GOSIGNE>
<SYNTAX $GOSIGNE = V-GOSIGNE>

<SYNTAX XGOSIGNF = V-GOSIGNF>
<SYNTAX $GOSIGNF = V-GOSIGNF>

<SYNTAX XGOSIGNG = V-GOSIGNG>
<SYNTAX $GOSIGNG = V-GOSIGNG>

<SYNTAX XGOSIGNH = V-GOSIGNH>
<SYNTAX $GOSIGNH = V-GOSIGNH>

<SYNTAX XGOSIGNI = V-GOSIGNI>
<SYNTAX $GOSIGNI = V-GOSIGNI>

<SYNTAX XGOSIGNJ = V-GOSIGNJ>
<SYNTAX $GOSIGNJ = V-GOSIGNJ>

<SYNTAX XGOSIGNK = V-GOSIGNK>
<SYNTAX $GOSIGNK = V-GOSIGNK>

<SYNTAX XGOSIGNL = V-GOSIGNL>
<SYNTAX $GOSIGNL = V-GOSIGNL>

<SYNTAX XGOSIGNM = V-GOSIGNM>
<SYNTAX $GOSIGNM = V-GOSIGNM>
;<SYNTAX XGONAVE = V-GONAVE>
;<SYNTAX $GONAVE = V-GONAVE>
;<SYNTAX XGOLAIR = V-GOLAIR>
;<SYNTAX $GOLAIR = V-GOLAIR>
;<SYNTAX XGOGOLD = V-GOGOLD>
;<SYNTAX $GOGOLD = V-GOGOLD>
;<SYNTAX XGOBAR  = V-GOGOLD>
;<SYNTAX $GOBAR  = V-GOGOLD>
;<SYNTAX XGOBANK = V-GOBANK>
;<SYNTAX $GOBANK = V-GOBANK>
;<SYNTAX XGOCLUB = V-GOCLUB>
;<SYNTAX $GOCLUB = V-GOCLUB>
;<SYNTAX XGOLIZA = V-GOLIZA>
;<SYNTAX $GOLIZA = V-GOLIZA>
;<SYNTAX XGOTOWR = V-GOTOWR>
;<SYNTAX $GOTOWR = V-GOTOWR>

<ROUTINE V-ENB-DEBUG ()
	<SETG GL-SPLIT-ROW 12>
	<SETG GL-SCORE-STS -1>
	<SETG GL-MOVES-STS -1>
	<SETG GL-PLACE-STS -1>
	<RT-INIT-SCREEN>
	<RT-UPDATE-STATUS-LINE>
	<RT-DESC-ALL>
	<SETG GL-DEBUG-FLAG T>
	<RFATAL>>

<ROUTINE V-GETLIT ()
	<TELL
"Poof! You now have \"the lamp\" and it is lit." CR CR
	>

	<MOVE ,TH-LAMP ,CH-PLAYER>
	<MC-MAKE ,TH-LAMP ,FL-LIGHTED>
	<RFATAL>>

<ROUTINE V-GOSIGNA ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-BROAD-SANCTUARY>
	<RFATAL>>

<ROUTINE V-GOSIGNB ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-MANUSCRIPT-ROOM>
	<RFATAL>>

<ROUTINE V-GOSIGNC ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-MADAME-TUSSAUDS>
	<RFATAL>>

<ROUTINE V-GOSIGND ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-GREAT-RUSSELL-ST>
	<RFATAL>>

<ROUTINE V-GOSIGNE ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-MONUMENT>
	<RFATAL>>

<ROUTINE V-GOSIGNF ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-THE-BLACK-MUSEUM>
	<RFATAL>>

<ROUTINE V-GOSIGNG ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-THREADNEEDLE-ST>
	<RFATAL>>

<ROUTINE V-GOSIGNH ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-TOWER-ENTRANCE>
	<RFATAL>>

<ROUTINE V-GOSIGNI ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-TRAFALGAR-SQUARE>
	<RFATAL>>

<ROUTINE V-GOSIGNJ ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-VICTORIA-STREET>
	<RFATAL>>

<ROUTINE V-GOSIGNK ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-SWAN-LANE>
	<RFATAL>>

<ROUTINE V-GOSIGNL ()
	<TELL "Poof! There's a sign here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-JERUSALEM-CHAMBER>
	<RFATAL>>

<ROUTINE V-GOSIGNM ()
	<TELL "Poof! There's a marquee here!" CR CR>
	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-THE-STRAND>
	<RFATAL>>

;<ROUTINE V-GONAVE ()
	<TELL
"Poof! You are now in the Nave." CR CR
	>

	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<RT-GOTO ,RM-NAVE>
	<RFATAL>>

;<ROUTINE V-GOLAIR ()
	<TELL
"Poof! You are now in the Lair." CR CR

"Moriarty, Akbar and Holmes are here. Holmes is tied up." CR CR>

	<RT-ALARM-CLR ,RT-I-PM-QUITS>
	<REMOVE ,TH-PHIAL>
	<MOVE ,TH-ROPE ,RM-LAIR>
	<MC-MAKE ,TH-ROPE ,FL-NODESC>
	<MC-MAKE ,CH-HOLMES ,FL-LOCKED>
	<MC-UNMAKE ,CH-HOLMES ,FL-ASLEEP>
	<SETG TH-ROPE-AUX1 ,CH-HOLMES>
	<SETG TH-ROPE-AUX2 -1>

	<MOVE ,CH-HOLMES ,RM-LAIR>
	<MOVE ,CH-AKBAR ,RM-LAIR>

	<SETG GL-PUPPY <>>

	<RT-ALARM-SET-REL ,RT-I-OUT-OF-TIME <RT-PARM-SET 0 50 0>>

	<RT-GOTO ,RM-LAIR>

	<RFATAL>>

;<ROUTINE V-GOGOLD ()
	<TELL
"Poof! You are now in the Bar of Gold." CR CR

"The carnation is in your lapel. You have the garnet. Holmes is not with
you." CR CR
	>

	<RT-ALARM-CLR ,RT-I-PM-QUITS>

	<MOVE ,TH-CARNATION ,TH-LAPEL>
	<MOVE ,TH-GARNET ,CH-PLAYER>

	<REMOVE ,TH-PHIAL>
	<MOVE ,TH-ROPE ,RM-LAIR>
	<MC-MAKE ,TH-ROPE ,FL-NODESC>
	<SETG TH-ROPE-AUX1 ,CH-HOLMES>
	<SETG TH-ROPE-AUX2 -1>
	<MC-MAKE ,CH-HOLMES ,FL-LOCKED>
	<MC-UNMAKE ,CH-HOLMES ,FL-ASLEEP>
	<MOVE ,CH-HOLMES ,RM-LAIR>

	<SETG GL-PUPPY <>>

	<RT-CLOCK-SET 2 0 0 20>

	<RT-GOTO ,RM-BAR-OF-GOLD>

	<RFATAL>>

;<ROUTINE V-GOBANK ()
	<TELL
"Poof! You are now in Threadneedle Street." CR CR

"You now have \"the lamp\" and it is lit. You also have the opal, the ruby,
the emerald, the sapphire and the safety deposit box key." CR CR
	> 

	<RT-ALARM-CLR ,RT-I-PM-QUITS>

	<MC-MAKE ,TH-LAMP ,FL-LIGHTED>
	<MOVE ,TH-LAMP ,CH-PLAYER>

	<MOVE ,TH-OPAL ,CH-PLAYER>
	<MOVE ,TH-RUBY ,CH-PLAYER>
	<MOVE ,TH-EMERALD ,CH-PLAYER>
	<MOVE ,TH-SAPPHIRE ,CH-PLAYER>
	<MOVE ,TH-BOX-KEY ,CH-PLAYER>
	<SETG CH-BANK-GUARD-AUX1 0>
	<RT-GOTO ,RM-THREADNEEDLE-ST>
	<RFATAL>>

;<ROUTINE V-GOCLUB ()
	<TELL
"Poof! You are now in the the Diogenes Club." CR CR

"You have the ring Sherlock gave you. Holmes is not with you." CR CR
	>

	<RT-ALARM-CLR ,RT-I-PM-QUITS>

	<REMOVE ,TH-PHIAL>
	<MOVE ,CH-HOLMES ,RM-LAIR>
	<MOVE ,TH-ROPE ,RM-LAIR>
	<MC-MAKE ,TH-ROPE ,FL-NODESC>
	<SETG TH-ROPE-AUX1 ,CH-HOLMES>
	<SETG TH-ROPE-AUX2 -1>
	<MC-MAKE ,CH-HOLMES ,FL-LOCKED>
	<MC-UNMAKE ,CH-HOLMES ,FL-ASLEEP>
	<MOVE ,TH-RING ,CH-PLAYER>

	<SETG GL-PUPPY <>>

	<RT-GOTO ,RM-DIOGENES-CLUB>

	<RFATAL>>

;<ROUTINE V-GOLIZA ()
	<TELL
"Poof! You are now in Covent Garden." CR CR
	>

	<RT-ALARM-CLR ,RT-I-PM-QUITS>

	<RT-GOTO ,RM-COVENT-GARDEN>

	<RFATAL>>

;<ROUTINE V-GOTOWR ("AUX" WORD-NUM)
	<TELL
"Poof! You are now in Byward Tower." CR CR

"You hear the Beefeater mumbling something about '"
	>
	<SET WORD-NUM <RANDOM 6>>
	<SETG PASSWORD-ID .WORD-NUM>
	<COND
		(<EQUAL? .WORD-NUM 1>
			<TELL "Boleyn">
		)
		(<EQUAL? .WORD-NUM 2>
			<TELL "Cleves">
		)
		(<EQUAL? .WORD-NUM 3>
			<TELL "Howard">
		)
		(<EQUAL? .WORD-NUM 4>
			<TELL "Parr">
		)
		(<EQUAL? .WORD-NUM 5>
			<TELL "Aragon">
		)
		(<EQUAL? .WORD-NUM 6>
			<TELL "Seymour">
		)
	>
	<TELL ".'" CR CR>

	<RT-ALARM-CLR ,RT-I-PM-QUITS>

	<RT-GOTO ,RM-BYWARD-TOWER>

	<RFATAL>>

>

<DEFAULT-DEFINITION COMMAND-CODE

<SYNTAX \#COMMAND = V-COMMAND>
<ROUTINE V-COMMAND ()
	<DIRIN 1>
	<RTRUE>>

<SYNTAX \#RECORD = V-RECORD>
<SYNTAX \#UNRECORD = V-UNRECORD>

<CONSTANT K-RECORD-ON 4>
<CONSTANT K-RECORD-OFF -4>

<ROUTINE V-RECORD ()
  <DIROUT ,K-RECORD-ON>
  T>

<ROUTINE V-UNRECORD ()
  <DIROUT ,K-RECORD-OFF>
  T>>

<ROUTINE V-VERIFY ()
	;<COND (<MC-T? ,GL-PRSO>
	       <COND (<AND <MC-PRSO? ,TH-INTNUM>
			   <EQUAL? ,GL-P-NUMBER 87>>
		      <TELL N ,SERIAL CR>)
		     (T
		      <TELL ,K-DONT-UNDERSTAND-MSG CR>)>
	       <RTRUE>)>
	<TELL "Verifying Game." CR CR>
	<COND (<VERIFY>
	       <TELL "Good." CR>)
	      (T
	       <TELL "Failed." CR>)>
	<RFATAL>>

<ROUTINE V-REFRESH ()
	<SETG GL-SPLIT-ROW  1>
	<RT-INIT-SCREEN>
	<RT-UPDATE-STATUS-LINE>
	<RT-DESC-ALL <> T>
	<RFATAL>>
