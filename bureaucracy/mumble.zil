...
<MSETG COMPUTER-HALF-HEIGHT </ ,COMPUTER-HEIGHT 2>>
<MSETG COMPUTER-HALF-WIDTH </ ,COMPUTER-WIDTH 2>>
...
<CONSTANT QTOPS
	  <PTABLE (BYTE) 0 0 COMPUTER-HALF-HEIGHT COMPUTER-HALF-HEIGHT>>
<CONSTANT QLEFTS
	  <PTABLE (BYTE) COMPUTER-HALF-WIDTH 0 0 COMPUTER-HALF-WIDTH>>
	...
	<REPEAT ((N <- 4 .QN>) RT TOP (CQ 3))
	  ...
	  <REPEAT (I (CT 0) TBL)
		  ...
		  <REPEAT ()
			  ...
			  <COND (<G=? <SET I <+ .I 1>> COMPUTER-HALF-HEIGHT>
				 <RETURN>)>>
		  <COND (<G=? <SET CT <+ .CT 1>> COMPUTER-HALF-WIDTH>
			 <RETURN>)>>
	  <SET CQ <- .CQ 1>>
...
<DEFINE GEN-BAD-CHAR (QN:FIX MB "AUX" BQ X Y)
  ...
  <COND (<==? .BQ .QN>
	 <SET BQ 0>)>
  <SET X <ZRANDOM COMPUTER-HALF-WIDTH>>
  <SET X <+ <SET X <- .X 1>> <GETB ,QLEFTS .BQ>>>
  <SET Y <ZRANDOM COMPUTER-HALF-HEIGHT>>
...
<DEFINE GEN-GOOD-CHAR (QN:FIX MB:FIX "AUX" GQ X Y (QCT 0))
  ...
  <SET X <ZRANDOM COMPUTER-HALF-WIDTH>>
  <SET X <- .X 1>>
  <SET Y <- <SET Y <ZRANDOM COMPUTER-HALF-HEIGHT>> 1>>
	       ...
	       <COND (<NOT .DUMP?>
			 ...
			 1 below"
		      ...
		      <COND (<==? <SET Y <GETB ,LAST-USED <+ .GQ*2 1>>>
				  255>
			     <SET Y 0>)>
		      <COND (<G=? .Y COMPUTER-HALF-HEIGHT>
			     <SET X COMPUTER-HALF-WIDTH>)>
		      <SET RX <+ .CL .X>>
		      <SET RY <+ .CT .Y>>)>
	       <PROG ()
	         <COND (<G? .QCT 5> <RETURN>)
		       (<G=? <SET X <+ .X 1>> COMPUTER-HALF-WIDTH>
		        ; "End of line, so go to next"
		        <COND (<G=? <SET Y <+ .Y 1>> COMPUTER-HALF-HEIGHT>
